...
$(function(){
	...
	$("#deleteMenuButton").click(function(){
		...
		$.messager.confirm('确认信息', '确认要删除吗?', function(r){
			if (r) {
				$.post(ctx + '/security/menu!deleteMenu.action', {id: nodes[0].id}, function(data){
					var n = eval("(" + data + ")");
	...
	$("#saveMenuButton").click(function() {
		...
		$.post(ctx + '/security/menu!saveMenuOrder.action', {'json2Order': jsonTree}, function(data){
			alert(data);
		});
	});
});
...
function recursiveTree2Order(nodes) {
	...
	for ( var i=0; i<nodes.length ;i++ ) {
		...
		jsonTree = jsonTree + "{\"id\": " + n.id + ", \"parent\":{\"id\": " + pid + "}, \"order\": " + zTree.getNodeIndex(n) + "},"
