...
<%@ attribute name="page" type="com.lxs.core.common.page.PageResult" required="true"%>
    ...
    var queryForm;
  	$(function() {
		...
		$('form').each(function() {
  		...
  		//如果不存在queryForm
  		if (!queryForm) {
  			$(document.body).append('<form id = "queryForm" method="post"></form>');
  			queryForm = $('#queryForm');
...
<c:set var="pageCountDouble" value="${m == 0 ? page.rowCount/pageSize : (page.rowCount - m)/pageSize + 1}"></c:set>
<fmt:formatNumber pattern="#" value="${pageCountDouble }" var="pageCount"></fmt:formatNumber>
